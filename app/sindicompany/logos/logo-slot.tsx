...
import { getLogoUploadIntent } from "../revista/nova/upload-actions";
...
export function LogoSlot({ slot, initialUrl }: LogoSlotProps) {
  ...
  async function handleFile(file: File) {
    ...
    if (file.size > 10 * 1024 * 1024) {
      setStatus("error");
      setErrorMsg("Logo maior que 10MB.");
      return;
    }
    setStatus("uploading");
    const ext = file.name.split(".").pop()?.toLowerCase() ?? "";
    const intent = await getLogoUploadIntent(slot, ext);
    if (!intent.ok) {
      setStatus("error");
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-onix-100 bg-white p-3">
      ...
      {/* Logos costumam ter aspect ratio variável (horizontal, quadrado,
          stacked). Damos uma área 4:3 com bg-contain pra exibir sem
          distorcer. */}
      <div
        className="aspect-[4/3] w-full rounded border border-onix-100 bg-onix-50 bg-center bg-no-repeat bg-contain"
