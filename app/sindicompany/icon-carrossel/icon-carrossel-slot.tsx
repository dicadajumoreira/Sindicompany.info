...
import { getIconCarrosselUploadIntent } from "../revista/nova/upload-actions";

const BUCKET = "condominios-fotos";

interface IconCarrosselSlotProps {
  slot: number;
  initialUrl?: string | null;
}

export function IconCarrosselSlot({ slot, initialUrl }: IconCarrosselSlotProps) {
  ...
  async function handleFile(file: File) {
    ...
    const intent = await getIconCarrosselUploadIntent(slot, ext);
    if (!intent.ok) {
      setStatus("error");
  ...
  // Convencao: slot N preenche o slide N+1 do carrossel
  const slidePreenchido = slot + 1;
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-onix-100 bg-white p-3">
      ...
      <div className="text-[10px] text-g60 -mt-1">
        Preenche o slide {slidePreenchido}
      </div>
      <div
